...
# 3D Unet Model is used for training and perdiction
...
def unet3d(pretrained_weights = None, input_size = (4,256,256,1), bn = True):
	inputs = Input(input_size)
    
	c1 = Conv3D(32, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (inputs)
	c1 = Conv3D(32, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (c1)
	if (bn):
		cb1 = BatchNormalization()(c1)
		p1 = MaxPooling3D((1, 2, 2)) (cb1)
	else:
		p1 = MaxPooling3D((1, 2, 2)) (c1)
	
	c2 = Conv3D(64, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (p1)
	c2 = Conv3D(64, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (c2)
	if (bn):
		cb2 = BatchNormalization()(c2)
		p2 = MaxPooling3D((1, 2, 2)) (cb2)
	else:
		p2 = MaxPooling3D((1, 2, 2)) (c2)

	c3 = Conv3D(128, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (p2)
	c3 = Conv3D(128, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (c3)
	p3 = MaxPooling3D((1, 2, 2)) (c3)
	if (bn):
		cb3 = BatchNormalization()(c3)
		p3 = MaxPooling3D((1, 2, 2)) (cb3)
	else:
		p3 = MaxPooling3D((1, 2, 2)) (c3)

	c4 = Conv3D(256, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (p3)
	c4 = Conv3D(256, (3, 3, 3), activation='relu', kernel_initializer='he_normal', padding='same') (c4)
	p4 = MaxPooling3D(pool_size=(1, 2, 2)) (c4)
	if (bn):
		cb4 = BatchNormalization()(c4)
		p4 = MaxPooling3D((1, 2, 2)) (cb4)
	else:
		p4 = MaxPooling3D((1, 2, 2)) (c4)
	p4 = Dropout(0.2)(p4)
	...
	u6 = Conv3D(256, (2, 2, 2), activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (1,2,2))(c5))
	...
	u7 = Conv3D(128, (2, 2, 2), activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (1,2,2))(c6))
	...
	u8 = Conv3D(64, (2, 2, 2), activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (1,2,2))(c7))
	...
	u9 = Conv3D(32, (2, 2, 2), activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (1,2,2))(c8))
	...
	outputs = Conv3D(3, (1, 1, 1), activation='softmax') (c9)

	model = Model(input = inputs, output = outputs)
	#model.compile(optimizer = Adam(lr = 5e-5), loss = combined_loss, metrics = [dice_coef])
